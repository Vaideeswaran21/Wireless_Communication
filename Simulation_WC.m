...
power_tx_busy_data = 2.24e-3; % Power consumption in transmit busy mode for data (22.4 mW)
...
plot(time_stamp_ds, cumulative_power_central_ds(1 : floor(length(cumulative_power_central_ds)/2)) , '-o', 'LineWidth', 2, 'DisplayName', 'IEEE801.11ba based HS Frame Format (Central)');
hold on;
plot(time_stamp_ds, cumulative_power_central_ds(floor(length(cumulative_power_central_ds)/2) + 1:end), '-o', 'LineWidth', 2, 'DisplayName', 'Customized HS Frame Format (Central)');
hold on;
plot(time_stamp_ds, cumulative_power_peripheral_ds(1 : floor(length(cumulative_power_peripheral_ds)/2)) , '-o', 'LineWidth', 2, 'DisplayName', 'IEEE801.11ba based HS Frame Format (Peripheral)');
hold on;
plot(time_stamp_ds, cumulative_power_peripheral_ds(floor(length(cumulative_power_peripheral_ds)/2) + 1:end), '-o', 'LineWidth', 2, 'DisplayName', 'Customized HS Frame Format (Peripheral)');
